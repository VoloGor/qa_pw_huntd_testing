...
Locator: locator('.CompaniesLanding_section__TBYlA').locator('h2.typography_h2__NcKnr')
Expected substring: "Hire handpicked A‑Player engineers across the globe"
Error: strict mode violation: locator('.CompaniesLanding_section__TBYlA').locator('h2.typography_h2__NcKnr') resolved to 3 elements:
    1) <h2 class="typography_h2__NcKnr mb-24 c-extradark-chocolate">Hire handpicked A‑Player engineers across the glo…</h2> aka getByRole('heading', { name: 'Hire handpicked A‑Player' })
    2) <h2 class="typography_h2__NcKnr mb-24">We were there</h2> aka getByRole('heading', { name: 'We were there' })
    3) <h2 class="cell large-6 large-offset-3 mb-48 typography_h2__NcKnr RealHiringBlock_title__pm5BE">This is what hiring should really look like</h2> aka getByRole('heading', { name: 'This is what hiring should' })

Call log:
  - Expect "toContainText" with timeout 5000ms
  - waiting for locator('.CompaniesLanding_section__TBYlA').locator('h2.typography_h2__NcKnr')
...
```ts
  ...
  3  | 
  4  | export class CompaniesHero extends BaseComponent {
  5  |   constructor(page) {
  6  |     super(page, page.locator('.CompaniesLanding_section__TBYlA'));
  7  | 
  8  |     this.title = this.host.locator('h2.typography_h2__NcKnr');
  9  |     this.subtitle = this.host.locator('p.typography_alertText__XN7_r');
  10 |     this.hireEngineersBtn = this.host.locator('a.GlobeBlock_heroButton__L_MEL');
  11 |   }
  12 | 
  13 |   async getTitleText() {
  14 |     return await this.title.textContent();
  15 |   }
  16 | 
  17 |   async getSubtitleText() {
  18 |     return await this.subtitle.textContent();
  19 |   }
  20 | 
  21 |   async verifyTitleContains(expectedText) {
> 22 |     await expect(this.title).toContainText(expectedText);
     |                              ^ Error: expect(locator).toContainText(expected) failed
  23 |   }
  24 | 
  25 |   async clickHireEngineers() {
  26 |     await this.hireEngineersBtn.click();
  27 |   }
  28 | }
  29 | 
```